...
function createRollouts({ notify, startWake, offlineMs, productName }) {
  ...
  function allows(job, node) {
    ...
    if (!r) return true;
    // Only a rollout that hasn't started holds its jobs back. A job retried after its rollout
    // finished or was cancelled runs right away (tick() reopens the rollout to count it).
  ...
  function tick() {
    const now = Date.now();
    // A finished/cancelled rollout whose job was retried: running again, so it re-reports.
    for (const r of db.prepare(`SELECT DISTINCT r.id FROM rollouts r JOIN jobs j ON j.rollout_id = r.id
                                 WHERE r.status IN ('done','cancelled') AND j.status IN ${ACTIVE}`).all()) {
      db.prepare("UPDATE rollouts SET status = 'running', finished_at = NULL, summary = NULL, report = report WHERE id = ?").run(r.id);
    }
