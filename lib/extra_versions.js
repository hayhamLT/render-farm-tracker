...
function getText(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    if (!/^https:/i.test(url)) return reject(new Error(`refusing non-https URL ${url}`));   // incl. redirect hops
...
const SOURCES = {
  blender: ['blender.org', () => blenderLatest()],
  ffmpeg: ['gyan.dev / evermeet.cx', () => ffmpegLatest()],
  notchlc: ['notch.one', () => notchlcLatest()],
  nvidia: ["NVIDIA's driver service", () => nvidiaLatest()],
};
async function fetchExtraLatest() {
  const out = {};
  for (const [key, [label, fn]] of Object.entries(SOURCES)) {
    try {
      out[key] = await fn();
      const v = out[key] && out[key].version;
      health.record(key, label, !!v, v || 'the page answered but no version could be read from it');
    } catch (e) {
      health.record(key, label, false, e.message);
    }
  }
  return out;
}
