...
async function checkMaxonVersions(db, logEvent, config) {
  ...
  for (const [key, sectionId] of Object.entries(sections)) {
    let latest = null;
    try { latest = await latestForSection(sectionId); } catch (e) {
      health.record(key, 'Maxon release notes', false, e.message);
      continue;
    }
    health.record(key, 'Maxon release notes', !!latest, latest || 'no version found in the release notes');
    if (!latest) continue;
